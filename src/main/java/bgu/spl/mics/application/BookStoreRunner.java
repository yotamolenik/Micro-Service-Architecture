...
public class BookStoreRunner {
    public static void main(String[] args) {
        ...
        JsonArray vehicles = initialResources.get(0).getAsJsonArray();
        ...
        try {
            customersFile = new FileOutputStream(args[1]);
            try {
        ...
        try {
            moneyRegisterFile = new FileOutputStream(args[4]);

        try {
            ObjectOutputStream output = new ObjectOutputStream(moneyRegisterFile);
            output.writeObject(MoneyRegister.getInstance());
            output.close();
            customersFile.close();
        } catch (FileNotFoundException e) {
            e.printStackTrace();
